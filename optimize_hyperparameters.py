...
def logging_process_target(logging_queue, shutdown_event):
    ...
    file_handler = logging.FileHandler(f"optimize_hyperparameters.log", mode="w")
...
if __name__ == '__main__':
    ...
    try:
        ...
        population_generation_settings = [r0486848.PopulationGenerationSettings(no_vertices * 2, 0, 0)]
        recombination_operators = [r0486848.RecombinationOperator.HGREX]
        elimination_schemes = [r0486848.EliminationScheme.LAMBDA_MU]
        no_islands = [7]
        island_swap_rates = [1]
        island_no_swapped_individuals = [1]
        default_mutation_chances = [0.05]
        mutation_chance_feedbacks = [False]
        mutation_chance_self_adaptivities = [False, True]
