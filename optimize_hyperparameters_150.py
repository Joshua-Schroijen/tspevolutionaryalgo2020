...
def logging_process_target(logging_queue, shutdown_event):
    ...
    file_handler = logging.FileHandler(f"optimize_hyperparameters_150.log", mode="w")
...
if __name__ == '__main__':
    ...
    output_file = open("r0486848_hyperparameter_optimization_results_150.txt", "w")
    ...
    try:
        ...
        logger.info(f'Starting hyperparameter optimization for instance size 150. PID = {os.getpid()}')
        np.random.seed(0)
        ...
        island_no_swapped_individuals = [1, 2, 4, 8, 16]
        default_mutation_chances = [0.05, 0.10, 0.25, 0.30, 0.35, 0.40]
